-- Fix storefront checkout unique email constraint exception handling
-- Ensures that guest and storefront order placements swallow customer_email_unique conflicts gracefully
-- so that orders are created 100% reliably without failing with:
-- duplicate key value violates unique constraint "customers_brand_normalized_email_unique"

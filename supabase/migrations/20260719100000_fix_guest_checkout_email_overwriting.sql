-- Migration: Secure Constraint-Safe Guest Checkout and Backend Guest Order Consolidation with precise email-matching to prevent guest email overwriting
-- Re-defines place_storefront_order_core to enforce email alignment before matching registered or legacy guest profiles by phone.
...
BEGIN
  ...
  -- Match by active user session id (explicitly logged in takes precedence)

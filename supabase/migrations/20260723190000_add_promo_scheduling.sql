-- Migration: Add scheduling and global redemption limits to promo_codes
ALTER TABLE public.promo_codes
  ADD COLUMN IF NOT EXISTS start_date timestamptz,
  ADD COLUMN IF NOT EXISTS end_date timestamptz,
  ADD COLUMN IF NOT EXISTS max_redemptions integer;

-- Update validation logic to check start/end dates and global redemption limits
...
BEGIN
  ...
  -- 2. Check scheduling start date
  IF v_promo.start_date IS NOT NULL AND now() < v_promo.start_date THEN
    RETURN jsonb_build_object('valid', false, 'reason', 'CODE_NOT_STARTED', 'start_date', v_promo.start_date);
  END IF;

  -- 3. Check scheduling end date
  IF v_promo.end_date IS NOT NULL AND now() > v_promo.end_date THEN
    RETURN jsonb_build_object('valid', false, 'reason', 'CODE_EXPIRED', 'end_date', v_promo.end_date);
  END IF;

  -- 4. Check global redemption limits
  IF v_promo.max_redemptions IS NOT NULL THEN
    SELECT count(*) INTO v_global_uses
    FROM public.orders o
    WHERE o.brand_id = v_brand_id
      AND o.promo_code_id = v_promo.id
      AND COALESCE(o.status, '') NOT IN ('cancelled', 'draft');
      
    IF v_global_uses >= v_promo.max_redemptions THEN
      RETURN jsonb_build_object('valid', false, 'reason', 'CODE_MAX_REDEMPTIONS_REACHED', 'max_redemptions', v_promo.max_redemptions);
    END IF;
  END IF;
  ...
  -- 6. Customer Constraints (first order & customer usage limit)

-- Keep courier delivery actions synchronized with the admin order, payment state,
-- and order activity timeline. This migration is intentionally safe to re-run.
...
ALTER TABLE public.business_settings
  ALTER COLUMN courier_out_for_delivery_message_en SET DEFAULT
    'Hi {{customer_name}}, your order #{{invoice_number}} from {{brand_name}} is now out for delivery.',
  ALTER COLUMN courier_out_for_delivery_message_ar SET DEFAULT
...
-- Remove the superseded overload so calls using the first three arguments cannot
-- resolve ambiguously against the new function whose last two arguments default.
...
BEGIN
  ...
  v_is_cod := lower(coalesce(v_order.payment_method, '')) IN
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'message_en', coalesce(
      nullif(btrim(v_settings.courier_out_for_delivery_message_en), ''),
      'Hi {{customer_name}}, your order #{{invoice_number}} from {{brand_name}} is now out for delivery.'
    ),
    'message_ar', coalesce(
      nullif(btrim(v_settings.courier_out_for_delivery_message_ar), ''),
      'مرحباً {{customer_name}}، طلبك رقم {{invoice_number}} من {{brand_name}} خرج الآن للتوصيل.'
    )
  );
END;
...
REVOKE ALL ON FUNCTION public.courier_update_delivery(uuid, text, text, boolean, numeric)
  FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.get_courier_delivery_message(uuid)
  FROM PUBLIC, anon;

GRANT EXECUTE ON FUNCTION public.courier_update_delivery(uuid, text, text, boolean, numeric)
  TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_courier_delivery_message(uuid)
  TO authenticated;
